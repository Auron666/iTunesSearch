...
class DataService {
                    ...
                    if let albumsResults = json["results"] as? NSArray {
                        for album in albumsResults {
                            if let albumInfo = album as? [String: AnyObject] {
                                ...
                                guard let artworkUrl = albumInfo["artworkUrl100"] as? String else {return}
                                ...
                                let albumInstance = Album(artistName: artistName, artworkUrl: artworkUrl, collectionId: collectionId, collectionName: collectionName, country: country, primaryGenreName: primaryGenreName, releaseDate: String(releaseDateFormatted))
    ...
    func getAlbumTracks (collectionId: Int, complition: @escaping ([Track]) -> ()) {
        ...
        print("Album Songs Url = \(ALBUM_SONGS_URL)")
